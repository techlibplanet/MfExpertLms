...
class NationalitySyncAdapter(context: Context, autoInitialize: Boolean, allowParallelSyncs: Boolean, database : MfExpertLmsDatabase, masterService: IMasters) : AbstractThreadedSyncAdapter(context, autoInitialize, allowParallelSyncs) {

    private val TAG = NationalitySyncAdapter::class.java.simpleName
    ...
    override fun onPerformSync(account: Account?, bundle: Bundle?, p2: String?, contentProvider: ContentProviderClient?, syncResult: SyncResult?) {
        try {
            Constants.logD(TAG, "Nationality onPerformSync for account[" + account?.name + "]")
            Constants.logD(TAG, "Nationality sync started and running...")
            ...
            if (messages.size == 0){
                Constants.logD(TAG, "Nationality sync success !")
                context.putPref(SharedPrefKeys.SP_NATIONALITY_SYNC_TIME, net.rmitsolutions.mfexpert.lms.Constants.getFormatDate())
                context.sendBroadcast(Intent(SyncSettingsViewHolder.ACTION_FINISHED_SYNC).putExtra("position", 16));
            }else{
//                NotificationHelper.notifyGroupedError(context, "Nationality Sync failed", messages.size.toString() + " Modules failed to sync", messages)
            }
        }finally {
            Constants.logD(TAG, "Nationality sync finally called...!")
        }
    }
}