package net.rmitsolutions.mfexpert.lms.sync.loanclosetype
...
class LoanCloseTypeSyncAdapter(context: Context, autoInitialize: Boolean, allowParallelSyncs: Boolean, database : MfExpertLmsDatabase, masterService: IMasters) : AbstractThreadedSyncAdapter(context, autoInitialize, allowParallelSyncs) {

    private val TAG = LoanCloseTypeSyncAdapter::class.java.simpleName
    ...
    override fun onPerformSync(account: Account?, bundle: Bundle?, p2: String?, contentProvider: ContentProviderClient?, syncResult: SyncResult?) {
        try {
            Constants.logD(TAG, "Loan close type onPerformSync for account[" + account?.name + "]")
            Constants.logD(TAG, "Loan close type sync started and running...")
            ...
            if (messages.size == 0){
                Constants.logD(TAG, "Loan Close Type sync success !")
                context.putPref(SharedPrefKeys.SP_LOAN_CLOSE_TYPE_SYNC_TIME, net.rmitsolutions.mfexpert.lms.Constants.getFormatDate())
                context.sendBroadcast(Intent(SyncSettingsViewHolder.ACTION_FINISHED_SYNC).putExtra("position", 13));
            }else{
//                NotificationHelper.notifyGroupedError(context, "Loan Close Type Sync failed", messages.size.toString() + " Modules failed to sync", messages)
            }
        }finally {
            Constants.logD(TAG, "Loan Close Type sync finally called...!")
        }
    }
}