...
class LiteracySyncService : Service() {
    ...
    override fun onCreate() {
        ...
        val depComponent = DaggerInjectServiceComponent.builder()
                ...
                .build()
        depComponent.injectLiteracySyncService(this)

        synchronized(syncAdapterLocker) {
            if (syncAdapter == null) {
//                syncAdapter = MastersSyncAdapter(applicationContext, true, false, roomDatabase, masterService)
                syncAdapter = LiteracySyncAdapter(applicationContext, true, false, database, mastersService)
                Log.d("Sync", "Literacy sync adapter created.")
            }
        }
    }
    ...
    companion object {
        ...
        private var syncAdapter: LiteracySyncAdapter? = null
    }
}