...
class LoanCloseTypeSyncService : Service() {
    ...
    override fun onCreate() {
        ...
        val depComponent = DaggerInjectServiceComponent.builder()
                ...
                .build()
        depComponent.injectLoanCloseTypeSyncService(this)

        synchronized(syncAdapterLocker) {
            if (syncAdapter == null) {
//                syncAdapter = MastersSyncAdapter(applicationContext, true, false, roomDatabase, masterService)
                syncAdapter = LoanCloseTypeSyncAdapter(applicationContext, true, false, database, mastersService)
                Log.d("Sync", "Loan Close Type sync adapter created.")
            }
        }
    }
    ...
    companion object {
        ...
        private var syncAdapter: LoanCloseTypeSyncAdapter? = null
    }
}