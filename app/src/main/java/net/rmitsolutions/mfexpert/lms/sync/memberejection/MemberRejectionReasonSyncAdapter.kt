...
class MemberRejectionReasonSyncAdapter(context: Context, autoInitialize: Boolean, allowParallelSyncs: Boolean, database : MfExpertLmsDatabase, masterService: IMasters) : AbstractThreadedSyncAdapter(context, autoInitialize, allowParallelSyncs) {

    private val TAG = MemberRejectionReasonSyncAdapter::class.java.simpleName
    ...
    override fun onPerformSync(account: Account?, bundle: Bundle?, p2: String?, contentProvider: ContentProviderClient?, syncResult: SyncResult?) {
        try {
            Constants.logD(TAG, "Member rejection reason onPerformSync for account[" + account?.name + "]")
            Constants.logD(TAG, "Member rejection reason sync started and running...")
            ...
            if (messages.size == 0){
                Constants.logD(TAG, "Member rejection reason sync success !")
                context.putPref(SharedPrefKeys.SP_MEMBER_REJECTION_REASON_SYNC_TIME, net.rmitsolutions.mfexpert.lms.Constants.getFormatDate())
                context.sendBroadcast(Intent(SyncSettingsViewHolder.ACTION_FINISHED_SYNC).putExtra("position", 15));
            }else{
//                NotificationHelper.notifyGroupedError(context, "Member Rejection Reason Sync failed", messages.size.toString() + " Modules failed to sync", messages)
            }
        }finally {
            Constants.logD(TAG, "Member Rejection Reason sync finally called...!")
        }
    }
}